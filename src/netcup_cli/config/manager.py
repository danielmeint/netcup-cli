...
class NetcupConfig(BaseModel):
    ...
    session_timeout: int = Field(
        default=900, description="Session timeout in seconds (15 minutes)"
    )
...
class ConfigManager:
    ...
    def get_config(self) -> NetcupConfig:
        ...
        if not customer_number:
            customer_number = keyring.get_password(
                self.KEYRING_SERVICE, "customer_number"
            )
        ...
        return NetcupConfig(
            ...
            api_endpoint=os.getenv(
                "NETCUP_API_ENDPOINT", NetcupConfig.model_fields["api_endpoint"].default
            ),
        )

    def save_credentials(
        self, customer_number: str, api_key: str, api_password: str
    ) -> None:
        """
