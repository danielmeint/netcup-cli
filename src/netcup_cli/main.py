...
@click.group()
@click.version_option(version="0.1.2", prog_name="netcup")
...
@auth.command()
@click.option(
    "--customer-number", prompt="Customer Number", help="Your netcup customer number"
)
...
def add(
    ...
    try:
        ...
        success = dns_manager.add_record(
            domain, hostname, record_type.upper(), destination, priority
        )
...
def update(
    ...
    try:
        ...
        success = dns_manager.update_record(
            domain, record_id, hostname, destination, priority
        )
...
def show() -> None:
    ...
    try:
        ...
        table.add_row(
            "API Key",
            (
                f"{config_obj.api_key[:8]}..."
                if len(config_obj.api_key) > 8
                else config_obj.api_key
            ),
        )
